...
if __name__ == '__main__':
    ...
    while acc_test<acc:     #训练达到指定精度停止
        ...
        idxs_users = np.random.choice(range(args.num_users), m, replace=False)  #在100个用户中随机选20个用户
        ...
        # r = [w_norm_list[i]/weight_time[i] for i in range(len(w_norm_list))]
        # print('比值',r)
        #使用在线背包挑用户
        ...
        for ind in index_t:
            ...
            # weight_time[ind] = train_time_list[ind] - cur_train_time + upload_time_list[ind] 
            #更新重量
            ...
            #在线背包选取过程
            if online_bag(w_norm_list[ind],weight_time[ind],zj):       #判断是否选
        ...
        print('背包物品重量：',weight_time)
        r = [w_norm_list[i]/weight_time[i] for i in range(len(w_norm_list))]
        print('用户的价值/重量比值：',r)
        print('进行在线背包的顺序：',idxs_users[index_t])
        print('挑选的用户为：',idxs_users[select_client],'数量为：',len(select_client))
        print('本轮所耗费时间:%d'%(z))

        #训练20个用二范数挑选10个进行FedAvg
        w_local = []
        for jj in select_client:
            w_local.append(w_locals[jj])
        ...
        # #确定准确率图像的纵轴的集合
        # net_glob.eval()
        # acc_train,l1  = test_img(net_glob, dataset_train, args)
        # acc_test,l2 = test_img(net_glob, dataset_test, args)
        # acc_train_set.append(acc_train)
        # acc_test_set.append(acc_test)
        # net_glob.train()    #开启训练模式
